...
-- In fact I'm not quite sure why it's 2-3 times slower, you get the same slow
-- down if you switch from indexing with (x, y, (z, (w, ())))
-- to (x, (y, (z, (w, ())))), which doesn't make sense to me.
-- ephemient had another lovely idea to generalize which I can hardly believe I
-- didn't think of, simply run both parts in 4d and set the size of the last
-- dimension to 1 for part 1

main :: IO ()
main = do
  input <- map (map (== '#')) . lines <$> readFile "input"
  let run' :: Int -> Int
      run' = withSNat (\d -> runST $ run d input 6)
  print $ run' 1
  print $ run' 2
...
withSNat :: forall a . (forall n . SNat n -> a) -> Int -> a
withSNat f = go SZ
  where go :: forall n . SNat n -> Int -> a
        go sn n | n <= 0    = f sn
                | otherwise = go (SS sn) (n - 1)
...
run d initial steps = do
  ...
  where
    ...
    go n s arrA arrB = do
      ...
      -- effectively shrink array to only cells that can be alive in principle
      ...
      forM_ (STA.range bounds) \i -> do
        ...
        neighSum <- sumTo 5 checks (STA.readArray arrA)
...
-- count up the sum but stop once it reaches n
sumTo :: forall a m . Monad m => Int -> [a] -> (a -> m Bool) -> m Int
sumTo n = go 0
  where
    go :: Int -> [a] -> (a -> m Bool) -> m Int
    go acc [] _ = pure acc
    go acc (x:xs) f | acc >= n = pure acc
                    | otherwise = do
                      b <- f x
